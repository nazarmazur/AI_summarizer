...
(function () {
  ...
  function pickTrack(tracks, preferLang) {
    ...
    // Always take the ORIGINAL track — we never ask YouTube to translate the
    // captions. Translated ("tlang") tracks are POT-blocked far more often, and
    // the AI writes the summary in the target language anyway (per the prompt).
    if (manual.length) return { track: manual[0], needsTranslation: false };
    if (asr.length)    return { track: asr[0],    needsTranslation: false };
    return null;
  }
  ...
  async function getTranscript(preferLang) {
    ...
    // Method B — POT wall: the direct fetch came back empty. Drive the player to
    // load the ORIGINAL track with its own proof-of-origin token and capture the
    // response. Verified live: this reliably pulls the auto-captions that other
    // summarizers also rely on — the earlier failures were from requesting the
    // *translated* track, which YouTube POT-blocks.
    if (!segments.length) {
