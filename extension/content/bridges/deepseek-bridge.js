// Bridge for chat.deepseek.com — types a prompt into the active chat,
// waits until streaming finishes, and returns the assistant message text.
(function () {
  'use strict';

  const SIG = '[AIS-DeepSeek-Bridge]';
  ...
  function lastAssistantBlock() {
    // DeepSeek renders assistant messages in containers with role="article" or similar
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg && msg.type === 'BRIDGE_RUN' && msg.provider === 'deepseek') {
      (async () => {
        try {
    ...
    if (msg && msg.type === 'BRIDGE_PING' && msg.provider === 'deepseek') {
