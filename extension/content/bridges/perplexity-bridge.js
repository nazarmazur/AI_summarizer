// Bridge for www.perplexity.ai — types a prompt into the active chat,
// waits until streaming finishes, and returns the assistant message text.
(function () {
  'use strict';

  const SIG = '[AIS-Perplexity-Bridge]';
  ...
  function lastAssistantBlock() {
    // Perplexity renders response in structured containers
    const msgs = document.querySelectorAll('[data-role="assistant"], .answer-block, [role="article"]');
    return msgs[msgs.length - 1] || null;
  }

  function isStillStreaming() {
    return !!document.querySelector('button[aria-label*="Stop" i]')
        || !!document.querySelector('[data-streaming="true"], .generating');
  }
  ...
  async function waitForCompletion() {
    ...
    throw new Error('Perplexity response timeout');
  }
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg && msg.type === 'BRIDGE_RUN' && msg.provider === 'perplexity') {
      (async () => {
        try {
    ...
    if (msg && msg.type === 'BRIDGE_PING' && msg.provider === 'perplexity') {
